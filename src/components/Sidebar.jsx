    ...
    <div className="relative bg-color">
      ...
      <div className={`fixed inset-y-0 left-0 z-30 transform ${isOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out md:relative md:translate-x-0 md:w-64 bg-color text-white `}>
        ...
        <ul>
          ...
          <li className="mb-2 p-1 pl-4 border-b-2 relative" onClick={()=>setOpen(!open)}><p className="text-xl cursor-pointer"><HiMiniUsers className='inline'/>  Manage User/School
          {
            open?<IoIosArrowUp className='inline'/>:<IoIosArrowDown className='inline'/>
          }</p>
          <ul className={`${!open ? "hidden" : "block"}`}>
            <li className='pl-10 pt-2'><Link to="/addschool" className='text-lg'>
            <IoIosArrowForward className="inline" />
            Add School</Link></li>
            <li className='pl-10 pt-2-'><Link to="/schoollist" className='text-lg'>
            <IoIosArrowForward className="inline" />
            School List</Link></li>
          </ul>
